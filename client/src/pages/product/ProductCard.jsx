...
const ProductCard = ({ product, showToast }) => {
  ...
  const navigateToInventory = (e) => {
    e.preventDefault();
    navigate(`/products/${product.id}/inventory`)
  }
        ...
        <CardFooter className="p-4 pt-0">
          {isAdmin(user.role) ?
            <div className="flex justify-between w-full mt-4 gap-4">
              <Button className='w-[50%] bg-blue-600' onClick={navigateToInventory}>
                New inventory
              </Button>
              <Button className='w-[50%]' variant="outline" onClick={navigateToEdit}>
                Edit Product
              </Button>
            </div>
            :
