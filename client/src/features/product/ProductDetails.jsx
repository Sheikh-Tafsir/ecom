            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
                <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start mb-20">
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-left duration-700">
                        <div className="relative group overflow-hidden rounded-[2.5rem] bg-white p-4 shadow-xl shadow-slate-200/50 border border-slate-100">
                            <Carousel className="w-full">
                                <CarouselContent>
                                    {product?.images?.map((image) => (
                                        <CarouselItem key={image.id}>
                                            <div className="aspect-[4/5] md:aspect-square flex items-center justify-center overflow-hidden rounded-3xl">
                                                <img
                                ...
                                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2">
                                    <CarouselPrevious className="relative left-0 translate-x-0 h-10 w-10 border-slate-200 bg-white/80 backdrop-blur-md hover:bg-white" />
                                    <CarouselNext className="relative right-0 translate-x-0 h-10 w-10 border-slate-200 bg-white/80 backdrop-blur-md hover:bg-white" />
                                </div>
                            </Carousel>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col animate-in fade-in slide-in-from-right duration-700">
                        <div className="mb-8">
                            <div className="flex items-center gap-3 mb-6">
                                {product?.categories?.map((category) => (
                                    <Badge key={category.id} variant="secondary" className="bg-blue-50 text-blue-700 hover:bg-blue-100 border-none px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">
                                        {category.name}
                                    </Badge>
                                ))}
                                {product?.quantity > 0 && (
                                    <Badge className="bg-emerald-50 text-emerald-700 border-none px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ml-auto">
                                        In Stock
                                    </Badge>
                                )}
                            </div>
                            ...
                            <div className="flex items-baseline gap-2 mb-8 p-6 rounded-[2rem] bg-white border border-slate-100 shadow-sm">
                                <span className="text-sm font-bold text-slate-400">$</span>
                                <span className="text-5xl font-black text-slate-900 tracking-tighter">{product?.price}</span>
                                <span className="text-xs font-bold text-slate-400 ml-1 uppercase tracking-widest">USD</span>
                            </div>
                        ...
                        <div className="space-y-8">
                            {product?.quantity > 0 ? (
                                <>
                                    <div className="flex items-center justify-between">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-xs font-medium text-slate-500">{product?.quantity} items available</p>
                                        </div>
                                        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl border border-slate-200/50">
                                            ...
                                            <span className="w-10 text-center font-black text-slate-800">{maxAvailable <= 0 ? 0 : quantity}</span>
                                            <Button
                                                variant="ghost"
                                                size="icon"
                                ...
                                <div className="p-8 rounded-[2.5rem] bg-red-50 border border-red-100 flex flex-col items-center text-center">
                                    ...
                                    <p className="text-sm font-medium text-red-700/70">This item is currently unavailable. Check back soon!</p>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="space-y-6">
                            {reviews?.length > 0 ? (
                                ...
                                <div className="bg-white rounded-3xl p-12 border border-slate-100 text-center shadow-sm">
                                    <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center text-slate-200 mx-auto mb-4">
                                        <Star className="h-8 w-8" />
                                    </div>
                                    <h3 className="text-lg font-bold text-slate-800 mb-1">No reviews yet</h3>
                                    <p className="text-sm font-medium text-slate-500">Be the first to share your thoughts about this product!</p>
                                </div>
                            )}
                        </div>
                    </div>
