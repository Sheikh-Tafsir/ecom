...
const ProductCard = ({product}) => {
    ...
    return (
        <Link to={`/products/${product.id}`} className="group h-full">
            <Card
                className="h-[500px] bg-white border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50  overflow-hidden transition-all duration-500 cursor-pointer flex flex-col">
                <div className="relative aspect-[4/5] overflow-hidden bg-slate-50">
                    ...
                    <div
                        className="absolute top-4 right-4 flex flex-col gap-2 translate-x-12 group-hover:translate-x-0 transition-transform duration-500">
                        {product.quantity > 0 ? (
                            <button
                            ...
                            <div
                                className="w-10 h-10 rounded-lg bg-red-500/90 backdrop-blur-md shadow-lg flex items-center justify-center text-white">
                                <ShoppingCart className="h-5 w-5 opacity-50"/>
                            </div>
                        )}
                        <button
                            className="w-10 h-10 rounded-lg bg-white/90 backdrop-blur-md shadow-lg flex items-center justify-center text-slate-400 hover:text-red-500 transition-all active:scale-90">
                    ...
                    {product.quantity <= 0 && (
                        <div
                            className="absolute inset-0 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center">
                            <span
                                className="px-4 py-2 rounded-full bg-white/90 text-red-600 text-[10px] font-black uppercase tracking-widest shadow-lg">
                                Out of Stock
                            </span>
                        </div>
                    )}
                    ...
                    <div className="flex items-end justify-between gap-4">
                        ...
                        <div className="flex flex-wrap justify-end gap-1 max-w-[50%]">
                            {product.categories?.slice(0, 1).map((category) => (
                                <Badge key={category}
                                       className="bg-blue-50 text-blue-700 hover:bg-blue-100 border-none px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest whitespace-nowrap">
                                    {category}
                                </Badge>
                            ))}
                        </div>
