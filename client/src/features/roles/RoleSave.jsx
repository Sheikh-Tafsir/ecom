...
const RoleSave = () => {
    ...
    const {handleSubmit, control, reset, setError, formState: {errors}} = useForm({
        resolver: zodResolver(RoleSchema), defaultValues: {
            name: '', permissions: []
        },
    });

    const {data: roleFromQuery, isLoading: isPageLoading} = useQuery({
        queryKey: queryKeys.roles.detail(id), queryFn: () => fetchRole(id), enabled: !!id && !roleFromState,
    });
    ...
    useEffect(() => {
        ...
        if (role) {
            reset({
                name: role.name.replace('ROLE_', ''), permissions: role.permissions
            });
        }
    ...
    return (<div className="min-h-[85vh] flex items-center justify-center">
        ...
        <Card className="w-full max-w-lg border-slate-100 shadow-xl shadow-slate-200/50 rounded-lg overflow-hidden">
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <CardContent className="space-y-6 pt-8 px-8">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                                <span
                                    className="bg-slate-100 px-3 h-11 flex items-center rounded-lg font-bold text-slate-400 border border-slate-200">ROLE_</span>
                            <Controller
                                name="name"
                                control={control}
                                render={({field}) => (<Input
                ...
                <CardFooter className="flex justify-end gap-3 px-8 pb-8 pt-4">
                    ...
                    {mutation.isPending ? (<ButtonLoading className="h-12 px-8 rounded-lg bg-blue-600 w-[140px]"/>) : (
                        <Button
                            type="submit"
