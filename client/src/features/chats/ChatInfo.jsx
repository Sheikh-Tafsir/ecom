...
import {CHAT_MEMBER_TYPE, CHAT_TYPE} from "@/constants/chat.constants"
        ...
        <div className="flex flex-col h-full bg-white animate-in slide-in-from-right duration-300">
            <div className="p-8 flex flex-col items-center border-b border-slate-50">
                <div className="relative mb-4 group">
                    <Avatar
                        className="h-24 w-24 border-4 border-slate-50 shadow-md transition-transform duration-300 group-hover:scale-105">
                        <AvatarImage src={image} alt={name}/>
                        <AvatarFallback
                            className="text-4xl bg-gradient-to-br from-blue-500 to-blue-600 text-white font-bold">
                            {name?.slice(0, 1)}
                        </AvatarFallback>
                    </Avatar>
                    <div
                        className="absolute bottom-1 right-1 w-5 h-5 bg-green-500 border-4 border-white rounded-full shadow-sm"/>
                </div>
                    ...
                    {type == CHAT_TYPE.GROUP ? (
                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Members
                                    ({participants?.length || 0})</h4>
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    ...
                                    <div
                                        ...
                                        <div className="flex items-center gap-3">
                                            <Avatar className="h-9 w-9 border border-white shadow-sm">
                                                ...
                                                <AvatarFallback
                                                    className="bg-slate-100 text-slate-600 text-xs font-medium">{participant.name?.slice(0, 1)}</AvatarFallback>
                                            </Avatar>
                                            <div className="flex flex-col">
                                                <span
                                                    className="text-sm font-semibold text-slate-800">{participant.name}</span>
                        ...
                        <div className="space-y-6">
                            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                                ...
                                <p className="text-xs text-slate-600 leading-relaxed">
                                    This is a secure direct conversation with {name}. Messages are end-to-end encrypted
                                    and visible only to you and them.
                                </p>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Button variant="ghost"
                                        className="w-full justify-start text-xs font-medium text-slate-700 hover:bg-slate-50 rounded-xl px-2 h-10">
                                    <span className="w-8 flex justify-center text-slate-400">🔔</span>
                                    Mute Notifications
                                </Button>
                                <Button variant="ghost"
                                        className="w-full justify-start text-xs font-medium text-red-600 hover:bg-red-50 rounded-xl px-2 h-10">
